...
def prepDataFrame(filename, k=4):
    ...
    df['store-1ea'] = df['store-1ea'].astype('float64')
    df['store-2ea'] = df['store-2ea'].astype('float64')
    ...
    # Add columns for the previous store values and their effective addresses
    ...
    df1['sea-1l'] = df['store-1ea'] % math.pow(2, 31) # lower 32 bits
    df1['sea-1u'] = (df['store-1ea'] - df1['sea-1l']) / math.pow(2, 31)
    df1['sea-2l'] = df['store-2ea'] % math.pow(2, 31) # lower 32 bits
    df1['sea-2u'] = (df['store-2ea'] - df1['sea-2l']) / math.pow(2, 31)
    ...
    df1['sea-1l'] = df1['sea-1l'] / df1['sea-1l'].max()
    df1['sea-1u'] = df1['sea-1u'] / df1['sea-1u'].max()
    df1['sea-2l'] = df1['sea-2l'] / df1['sea-2l'].max()
    df1['sea-2u'] = df1['sea-2u'] / df1['sea-2u'].max()

    # Reorder the columns so the 'outputs' are the last 2 columns
    df1 = df1[["pc-l", "ea-l", "ea-u", "v0l-1", "v0u-1", "v0l-2", "v0u-2", "v0l-3", \
               "v0u-3", "v0l-4", "v0u-4", "s-1l", "s-1u", "s-2l", "s-2u", 'sea-1l', \
               'sea-1u', 'sea-2l', 'sea-2u', "v0-l", "v0-u"]]
...
def build_model(do1=100, do2=100, do3=100, do4=100, do5=100, l_rate=0.00075):
    model = Sequential()
    model.add(Dense(do1,input_dim=19, kernel_initializer='normal', activation='relu'))
