...
int main(int argc, char** argv) {
	...
	out_file << "pc,effective_address,val0,store-1ea,store-1val0,store-2ea,store-2val0\n";

	uint64_t num_instrs = 0; // Number of instructions processed
	cout << "Reading from " << trace_name << "...\n";

	bool seen_prev_store1 = false;
	bool seen_prev_store2 = false;
	uint64_t prev_store1val = DEADBEEF;
	uint64_t prev_store1eff = DEADBEEF;
	uint64_t prev_store2eff = DEADBEEF;
	uint64_t prev_store2val = DEADBEEF;
	...
	while(!d_input->eof()) {
		...
		if (iType == InstClass::loadInstClass) {
			...
			out_file << pc << "," << effAddr << "," << outValVec[0] << ",";
			
			if (seen_prev_store1) out_file << prev_store1eff << "," << prev_store1val << ",";
			
			else out_file << "NAN,NAN,";

			if (seen_prev_store2) out_file << prev_store2eff << "," << prev_store2val << "\n";

			else out_file << "NAN,NAN\n";
		}
		else if (iType == InstClass::storeInstClass) {

			if (seen_prev_store1)
			{
				// Update store before the previous store
				prev_store2eff = prev_store1eff;
				prev_store2val = prev_store1val;
				seen_prev_store2 = true;
			}
			// Update the previous store
			prev_store1eff = effAddr;
			prev_store1val = outValVec[0];
			seen_prev_store1 = true;
		}
